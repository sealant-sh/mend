...
output "fsx_dns_name" { value = aws_fsx_openzfs_file_system.store.dns_name }
output "fsx_file_system_id" { value = aws_fsx_openzfs_file_system.store.id }
# OpenZFS mounts the root volume at /fsx; child volumes are exported under it.
output "fsx_mend_volume_path" { value = "/fsx/${aws_fsx_openzfs_volume.mend.name}" }
