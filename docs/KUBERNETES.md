...
with the Sealant chart is gone. The incident class the shared filesystem produced (uid split, root
`gc` poisoning, stale sockets, `PLATFORM-FEEDBACK.md` 2026-08-29/30) disappears by construction;
failure modes are rows in Postgres.
...
- The **web tier scales freely** (`web.replicaCount`): it is a stateless TanStack server plus a
  ...
  to the same API.
- A workspace Pod that vanishes without an exit — node loss, OOM kill,
  `kubectl delete pod --grace-period=0 --force` — is a pickup (ADR-0002 "Replacement and pickup"):
  its lease lapses within 30 s, Mend confirms the Pod is gone, settles the session
  `executor lost · lease expired`, and the next resume claims the next epoch and launches a
  replacement that materialises the newest capture whose git section verifies and resumes the
  harness by provider session id. Work since the last capture — at most the small-class cadence, 2 s
  quiet / 10 s while dirty — is gone with the process. Pod recovery is "resume from the last
  capture", not "migrate a live process".
- A plain `kubectl delete pod` is a planned stop, not a loss: the Pod gets SIGTERM, sealantd flushes
  a `final` capture, the harness exits, and the session settles `completed` through the run's exit
  before the lease can lapse. Resume it as any settled session. To rehearse a pickup, force the
  delete.
...
Then point the chart at the claim's outputs. `endpoint` is the egress rule the API tier gets. Two
ports live there: `port` (80) is the gateway **Service** port, the one `BUCKET_PORT` and the URL
carry; `podPort` (8080) is what the RGW **container** listens on (Rook's `rgwPortInternalPort`, the
Service maps 80 → 8080). A NetworkPolicy matches the pod port — the rule is evaluated after the
Service's DNAT — so every egress rule to the bucket, here and on the Sealant side, names 8080, while
every URL names 80. The RGW Pods carry `app: rook-ceph-rgw` and `rook_object_store: <store>` if you
want the rule narrower than the namespace. `store` is an explicit choice: `create` for a fresh
install, `existingClaim` for an upgrade ("Upgrade" below); the chart refuses to render with neither.
...
captureStore:
  blobStore:
    ...
    # Garage's container listens on 3900 itself, so the Service port and the pod port agree.
    endpoint:
...
networkPolicies:
  workspaceEgressAllow:
    ...
    - namespaceSelector: { matchLabels: { kubernetes.io/metadata.name: rook-ceph } }
      ...
      port: 8080 # the RGW POD port, not the Service's 80 (see above)
workspaces:
...
3. Set `store.existingClaim: mend-store` (and leave `store.create.enabled` off): the old RWX claim
   still holds the bare repositories, the runner cache, the machine key and every worktree
   directory, and an RWX claim mounts fine as the API Pod's only volume. This is why the chart has
   no default for the store — `create` on an upgrade would mount a new, empty claim and the backfill
   below would find nothing. Run `helm upgrade`. A worktree that still has a directory on the claim
   is **backfilled at its first launch** (#235, ADR-0002 decision 24): capture 0 is a final
   co-located checkpoint of the directory's current files, uncommitted edits included, so the work
   rides into the bucket and the directory is not read again. No separate migrate step.
...
| `kubectl delete pod` on a workspace settled the session `completed`, not `executor lost` | A graceful delete is SIGTERM: sealantd flushes a `final` capture and the harness exits — a planned stop. Only a Pod that vanishes without an exit (`--grace-period=0 --force`, node loss) takes the lease-expiry path and is picked up on resume.                                                                                                                                                              |
| The API log says `git section failed verification · observed` for a capture              | The executor's pack omits an object its refs name (`git rev-list --missing=error` on the runner said which). The capture row is `git_fsck = failed`; plans and reads use the newest verified capture below it and say so (`plan restores an older git section`, `reading the newest verified capture`). Edits in the failed capture are in the bucket but unreadable until the executor ships a complete pack. |
| `review prep: the change could not be read · no passes queued`                           | The log line carries `git`, `stderr`, `captureN` and `gitFsck`: the chain head could not be diffed on the runner. The review page still offers both passes on demand; see the row above for a `failed` head.                                                                                                                                                                                                   |
