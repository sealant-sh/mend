...
| Account-isolated multi-user deployment                                                                                   | **Shipped (teams, docs/adr/0002)**             | Project scope (personal · team · instance) checked on every project-scoped route, the terminal, the Service tunnel, and the event stream; repo and migration tests cover seats, invites, and scope. Two-user HTTP tests are still to add. | State that an account sees instance projects plus its own and its teams'; everything else answers 404.                           |
