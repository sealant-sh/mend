...
const contract = (name: string, layerOf: () => Layer.Layer<BlobStore>, prefix: string) => {
  describe(name, () => {
...
describe("BlobStore (dir)", () => {
  ...
  contract("contract", () => BlobStoreFsLive(root), "t");
  ...
  it("leaves no temp files behind and never lists them", async () => {
