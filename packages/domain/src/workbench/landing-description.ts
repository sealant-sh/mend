 ...
 * "The pull request's description"). Pure: the landing service gathers the facts and these
 * render them, and the web's Land panel renders the same preview from the facts it has.
 *
...
/**
 * How git names a file's change in a range, as `git diff --name-status` spells it. Kept here, not
 * in the store, so every client can render a description preview.
 */
export type DescribedFileStatus =
  | "added"
  | "modified"
  | "deleted"
  | "renamed"
...
export interface DescribedFile {
  ...
  readonly status: DescribedFileStatus;
...
export interface DescriptionInput {
  ...
  /**
   * The landed checkpoint; null in a preview, before the landing has taken it. The section then
   * says it is taken when the change lands.
   */
  readonly checkpoint: { readonly ordinal: number; readonly sha: string } | null;
...
const renderSection = (input: DescriptionInput, fileLimit: number): string => {
  ...
  return [
    ...
    input.checkpoint === null
      ? "- Landed checkpoint: taken when this lands"
      : link(
          "Landed checkpoint",
          input.links.checkpoint,
          `checkpoint ${input.checkpoint.ordinal} · ${input.checkpoint.sha.slice(0, 7)}`,
        ),
    "",
