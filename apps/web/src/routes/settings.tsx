  ...
  return (
    <AppShell>
      ...
      <div className="space-y-6">
        ...
        <InstanceDefaults view={organization}>
...
/** The instance's workspace environment: what an organization that set none of its own inherits. */
  ...
  return (
    <section className="rounded-2xl bg-panel p-6 shadow-[var(--shadow-sm)]">
      ...
      <p className="mt-1 text-sm leading-relaxed text-muted-foreground">
        Applied whenever a session starts or resumes. Workspaces already running are unchanged. An
        organization can set its own, and a project can override either.
      </p>
      <WorkspaceEnvironmentEditor
        savedImage={settings.workspaceImage}
        allowScan
        onSave={async (image) => {
          const result = await saveWorkspaceEnvironment(image);
          if (!result.saved) return { saved: null, resolutions: result.resolutions };
          ...
          void queryClient.invalidateQueries(trpc.organization.settings.queryFilter());
          return { saved, resolutions: result.resolutions };
        }}
      />
    </section>
  );
...
function SessionLifecyclePanel() {
  ...
  const toggle = (value: boolean) => {
    ...
    void putSettings(next)
      .then(() => invalidateDefaults(queryClient, trpc))
...
function ReviewAutomationPanel() {
  ...
  return (
    <section className="rounded-2xl bg-panel p-6 shadow-[var(--shadow-sm)]">
      ...
      <p className="mt-1 text-sm text-muted-foreground">
        The machine-side passes around a session. Defaults for every organization; an organization
        can set its own, and a project can override any switch on its own page.
      </p>
      <div className="mt-5 space-y-5 border-t border-[var(--sw-faint-rule)] pt-5">
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="min-w-0">
            ...
            <p className="mt-1 text-[13px] leading-relaxed text-muted-foreground">
              ...
              default for every organization; an organization can set its own, a project can
              override it on its setup page, and a session when it starts. Sessions started from
              Slack follow the Slack app&apos;s setting. Terminal sessions never land by themselves;
              land them from the change page.
            </p>
          </div>
          <div className="flex shrink-0 gap-2">
            {[true, false].map((value) => (
